...
async function ina(method, path, query, payload){   
    ...
    else
   {
    ...
    await axios(url, {
      ...
      console.log('with url: ${url} and ${path} and ${method}');
      return JSON.stringify(error);
    });
    }
...
module.exports = { 
  ...
  main: function (event, context) {
    ...
    //return event.extensions.request.path;   
    //return event.extensions.request.query.schema;
    //return event.extensions.request.query; //sap/bc/ina/service/v2/GetResponse?Request={"Metadata":{"Expand":["Cubes"]}}
    let func = ina;
    switch (event.extensions.request.query.schema) {
      case "1":
        return hanacloud();
        break;
      case "2":
    ...
    //return event.extensions.request.body.data;
    
    return func(event.extensions.request.method, event.extensions.request.path, event.extensions.request.query, event.data);
